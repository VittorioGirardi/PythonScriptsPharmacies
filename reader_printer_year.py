...
class Farmacia:
    ...
    def prompt_user_for_selections(self):
        ...
        while self.ListaFarmacie:
            ...
            for mese in self.ListaMesi:
                print_options(self.ListaOrari)
                selected_orario_index = get_valid_input(f"\nInserisci il numero corrispondente all'Orario per {mese}: ", range(1, len(self.ListaOrari) + 1))
                selected_orario = self.ListaOrari[selected_orario_index - 1]
                selected_mese = self.ListaMesi.index(mese) + 1
